...
const sendHeartbeat = () => {
  emitEvent("heartbeat", "extension_host_heartbeat")
}

const sendReady = () => {
  emitEvent("ready", "extension_host_ready")
}

const sendInitialized = () => {
  emitEvent("initialized", "extension_host_initialized")
}
...
const handle = async (req: RuntimeRequest) => {
  ...
  if (!initialized) {
    respond(req, false, null, "extension-host is not initialized")
    return
  }
